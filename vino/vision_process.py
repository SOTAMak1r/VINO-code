...
def smart_nframes(
    ...
    if "nframes" in ele:
        ...
        max_frames = min(max_frames, 8) # hard code
        nframes = total_frames / video_fps * fps
        if nframes > total_frames:
            logger.warning(f"smart_nframes: nframes[{nframes}] > total_frames[{total_frames}]")
        nframes = min(min(max(nframes, min_frames), max_frames), total_frames)
        nframes = floor_by_factor(nframes, FRAME_FACTOR)
        # print(f'[nframes, min_frames, max_frames, total_frames, video_fps, fps] {nframes, min_frames, max_frames, total_frames, video_fps, fps}')
...
def _read_video_decord(
    ...
    print(f'[start_frame, end_frame, total_frames] {start_frame, end_frame, total_frames}')
    # nframes = smart_nframes(ele, total_frames=total_frames, video_fps=video_fps)
    nframes = 8 # hard code
    ...
    print(f'[qwen3vl] idx = {idx}')
    # print(f'[debug] {nframes, idx, sample_fps, video_fps, total_frames}')
...
def fetch_video(ele: Dict[str, Any], image_patch_size: int = 14, return_video_sample_fps: bool = False,
    ...
    print(f'[final_video] shape = {video.shape}')
